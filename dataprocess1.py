...
class EdgeSampler(data_utils.Dataset):
    ...
    """
    def __init__(self, A, A_fadj , A2, A2_fadj, num_pos=1000, num_neg=1000):
        self.num_pos = num_pos
        self.num_neg = num_neg
        self.A = A
        self.A_fadj = A_fadj
        self.A2 = A2
        self.A2_fadj = A2_fadj

        self.edges_adj = np.transpose(A.nonzero())
        self.edges_fadj = np.transpose(A_fadj.nonzero())
        self.edges_adj2 = np.transpose(A2.nonzero())
        self.edges_fadj2 = np.transpose(A2_fadj.nonzero())

        self.num_nodes = A.shape[0]
        self.num_edges_adj = self.edges_adj.shape[0]
        self.num_edges_fadj = self.edges_fadj.shape[0]
        self.num_edges_adj2 = self.edges_adj2.shape[0]
        self.num_edges_fadj2 = self.edges_fadj2.shape[0]
    ...
    def __getitem__(self, key):
        ...
        edges_idx3 = np.random.randint(0, self.num_edges_adj2, size=self.num_pos, dtype=np.int64)
        next_edges_adj2 = self.edges_adj2[edges_idx3, :]

        edges_idx4 = np.random.randint(0, self.num_edges_fadj2, size=self.num_pos, dtype=np.int64)
        next_edges_fadj2 = self.edges_fadj2[edges_idx4, :]
        generated1 = False
        generated2 = False
        generated3 = False
        generated4 = False
        ...
        while not generated3:
            candidate_ne_adj2 = np.random.randint(0, self.num_nodes, size=(2*self.num_neg, 2), dtype=np.int64)
            # candidate_ne_fadj = np.random.randint(0, self.num_nodes, size=(2*self.num_neg, 2), dtype=np.int64)

            cne1_a, cne2_a = candidate_ne_adj2[:, 0], candidate_ne_adj2[:, 1]
            # cne1_f, cne2_f = candidate_ne_fadj[:, 0], candidate_ne_fadj[:, 1]

            to_keep_adj2 = (1 - self.A2[cne1_a, cne2_a]).astype(bool).A1 * (cne1_a != cne2_a)
            # to_keep_fadj = (1 - self.A[cne1_f, cne2_f]).astype(np.bool).A1 * (cne1_f != cne2_f)

            next_nonedges_adj2 = candidate_ne_adj2[to_keep_adj2][:self.num_neg]
            # next_nonedges_fadj = candidate_ne_fadj[to_keep_fadj][:self.num_neg]
            # next_nonedges = np.vstack((next_nonedges_adj , next_nonedges_fadj))
            # np.random.shuffle(next_nonedges)
            generated3= to_keep_adj2.sum() >= self.num_neg

        while not generated4:
            candidate_ne_fadj2 = np.random.randint(0, self.num_nodes, size=(2*self.num_neg, 2), dtype=np.int64)
            # candidate_ne_fadj = np.random.randint(0, self.num_nodes, size=(2*self.num_neg, 2), dtype=np.int64)

            cne1_a, cne2_a = candidate_ne_fadj2[:, 0], candidate_ne_fadj2[:, 1]
            # cne1_f, cne2_f = candidate_ne_fadj[:, 0], candidate_ne_fadj[:, 1]

            to_keep_fadj2 = (1 - self.A2_fadj[cne1_a, cne2_a]).astype(bool).A1 * (cne1_a != cne2_a)
            # to_keep_fadj = (1 - self.A[cne1_f, cne2_f]).astype(np.bool).A1 * (cne1_f != cne2_f)

            next_nonedges_fadj2 = candidate_ne_fadj2[to_keep_fadj2][:self.num_neg]
            # next_nonedges_fadj = candidate_ne_fadj[to_keep_fadj][:self.num_neg]
            # next_nonedges = np.vstack((next_nonedges_adj , next_nonedges_fadj))
            # np.random.shuffle(next_nonedges)
            generated4= to_keep_fadj2.sum() >= self.num_neg
            # generated_fadj = to_keep_fadj.sum() >= self.num_neg

        return (torch.LongTensor(next_edges_adj), torch.LongTensor(next_edges_fadj) , torch.LongTensor(next_edges_adj2), torch.LongTensor(next_edges_fadj2),
                torch.LongTensor(next_nonedges_adj) ,  torch.LongTensor(next_nonedges_fadj), torch.LongTensor(next_nonedges_adj2) ,  torch.LongTensor(next_nonedges_fadj2))

    def __len__(self):
        return 2**32

def collate_fn(batch):
    edges1, edges2 , edges3, edges4,  nonedges1 , nonedges2, nonedges3, nonedges4 = batch[0]
    return (edges1, edges2 , edges3, edges4,  nonedges1 , nonedges2, nonedges3, nonedges4)

def get_edge_sampler(A , A_fadj, A2, A2_fadj, num_pos=1000, num_neg=1000, num_workers=5):
    data_source = EdgeSampler(A , A_fadj, A2, A2_fadj,  num_pos, num_neg)
...
def load_cora(data1 , data2, N):
    ...
    return sadj, fadj, adj_lable, A, A_fea, fadj_label
